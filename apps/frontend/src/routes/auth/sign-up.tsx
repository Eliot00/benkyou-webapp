...
import { signIn, signUp } from "~/libs/better-auth";
...
export default function SignUp() {
	...
	return (
		<main class="h-vh w-full flex items-center justify-center bg-background">
			<Card>
				<CardHeader>
					<CardTitle class="text-center text-2xl font-bold">Sign up</CardTitle>
					<CardDescription class="text-center">
						Create an account to start learning Japanese
					</CardDescription>
				</CardHeader>
				<CardContent class="space-y-6">
					<form class="space-y-6" onSubmit={handleSubmit}>
						<TextFieldRoot name="name" validationState={error() ? "invalid" : "valid"} required>
							<TextField type="text" placeholder="Your name" />
						</TextFieldRoot>
						<TextFieldRoot name="email" validationState={error() ? "invalid" : "valid"} required>
							<TextField type="email" placeholder="example@example.com" />
						</TextFieldRoot>
						<TextFieldRoot name="password" validationState={error() ? "invalid" : "valid"} required>
				...
				<CardFooter class="flex justify-center">
					<p class="text-sm text-gray-600">
						...
						<A href="/auth/sign-in" class="text-primary hover:underline">
							Sign in
						</A>
					</p>
				</CardFooter>
